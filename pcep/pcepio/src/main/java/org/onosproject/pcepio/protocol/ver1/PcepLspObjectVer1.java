...
public class PcepLspObjectVer1 implements PcepLspObject {
    ...
    /*
     ...
     在一个PCRpt消息中，C设置为1表示LSP是通过PCInitiate消息创建的。在LSP持续
     存在的时间段内，每个PCRpt消息中这个LSP Object的该标志位C都 MUST 设置为1。
     C标志位使得PCEs能够判别LSPs是否是PCE-initiated的。

     在ACTN中，标志位C应该总是1才对，因为只有一种途径能够创建LSP。
     */
    ...
    public static final short LSP_OBJ_MINIMUM_LENGTH = 8;

    public static final int DEFAULT_PLSPID = 0;
    public static final byte DEFAULT_OFLAG = 1;
    public static final boolean DRFAULT_CFLAG = false;
    ...
    public static final PcepObjectHeader DEFAULT_LSP_OBJECT_HEADER = new PcepObjectHeader(LSP_OBJ_CLASS, LSP_OBJ_TYPE,
    ...
    protected int packOptionalTlv(ChannelBuffer cb) {
        // From yby. 和印研沟通，确认llOptionalTlv不是必须的，并且在构造的时候这个成员总是null，因此做了null的判断
        // 初步判断原因是ONOS官方代码的缺陷。
